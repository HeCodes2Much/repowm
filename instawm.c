...
void
runAutostart(void) {
	system("command -v instanwm-schemas || { sleep 4 && notify-send 'instanwm-schemas missing, please install instanwm-schemas!!!'; } &");
}
...
resource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst)
{
	...
	snprintf(fullname, sizeof(fullname), "%s.%s", "instawm", name);
