...
struct Client {
    ...
    int bw, oldbw;
    unsigned int tags;
    int isfixed, iscentered, switchtotag, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow;
    int issteam;
    pid_t pid;
    Client *next;
    Client *snext;
...
drawbar(Monitor *m)
{
    ...
    drw_setscheme(drw, scheme[SchemeNorm]);
    x = drw_text(drw, x, 0, w, bh, (w - TEXTW(m->ltsymbol)) * 0.5 + 10, m->ltsymbol, 0);
